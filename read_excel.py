...
def main():
    ...
    rows = [x[TABLE_START_INDEX:] for x in rows
            if x[TEST_COL_INDEX + TABLE_START_INDEX:]]
...
def print_test_procedure(rows):
    ...
    for y in range(0, len(rows)):
        for x in range(CONDITION_COL_INDEX, TEST_COL_INDEX - TABLE_START_INDEX + 1):
            if not rows[y][x] and (x == CONDITION_COL_INDEX or
                                   rows[max(y - 1, 0)]
                                   [max(x - 1, CONDITION_COL_INDEX)] == rows[y][max(x - 1, CONDITION_COL_INDEX)]):
                rows[y][x] = rows[max(y - 1, 0)][x]

    # テスト項目毎に
    for test_no in range(TEST_COL_INDEX - TABLE_START_INDEX + 1, len(rows[0])):
        ...
        for row in rows:
            ...
            if row[test_no] == "-":
                continue
            print(f"　　{row[test_no]}: {row[CONDITION_COL_INDEX]}", end="")
            for cond_index in range(CONDITION_COL_INDEX + 1, TEST_COL_INDEX):
                if row[cond_index]:
